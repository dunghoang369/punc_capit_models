...
def main():
    ...
    label_list_capit, label_list_punc = processor.get_labels()[0], processor.get_labels()[1]

    num_labels_capit = len(label_list_capit) + 1
    num_labels_punc = len(label_list_punc) + 1
    ...
    # creating model
    model = XLMRForTokenClassification(pretrained_path=args.pretrained_path, n_capit_labels=num_labels_capit,
                                       n_punc_labels=num_labels_punc, hidden_size=hidden_size,
    ...
    if args.do_train:
        train_features = convert_examples_to_features(
            train_examples, label_list_capit, label_list_punc, args.max_seq_length, model.encode_word)
        ...
        val_features = convert_examples_to_features(
            val_examples, label_list_capit, label_list_punc, args.max_seq_length, model.encode_word)
        ...
        for _ in tqdm(range(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tbar):
                ...
                input_ids, label_ids_capit, label_ids_punc, l_mask, valid_ids = batch
                loss = model(input_ids, label_ids_capit, label_ids_punc, l_mask, valid_ids)
