...
def evaluate_model(model, eval_dataset, label_list_capit, label_list_punc, batch_size, device):
     """
     ...
     for input_ids, label_ids_capit, label_ids_punc, l_mask, valid_ids in eval_dataloader:

          input_ids = input_ids.to(device)
          label_ids_capit = label_ids_capit.to(device)
          label_ids_punc = label_ids_punc.to(device)
          ...
          with torch.no_grad():
               # logits = model(input_ids, labels=None, labels_mask=None,
                              ...
                              # input_ids, label_ids_capit, label_ids_punc, l_mask, valid_ids
               logits_capit, logits_punc = model(input_ids, None, None, None,
                              valid_mask=valid_ids)

          logits_capit = torch.argmax(logits_capit, dim=2)
          logits_capit = logits_capit.detach().cpu().numpy()
          logits_punc = torch.argmax(logits_punc, dim=2)
          logits_punc = logits_punc.detach().cpu().numpy()
          label_ids_capit = label_ids_capit.cpu().numpy()
          label_ids_punc = label_ids_punc.cpu().numpy()

          for i, cur_label in enumerate(label_ids_capit):
               temp_1 = []
               temp_2 = []

               for j, m in enumerate(cur_label):
                    if valid_ids[i][j]:  # if it's a valid label
                         temp_1.append(label_map_capit[m])
     ...
     # f1 = f1_score(y_true, y_pred, average='Macro')
          ...
          for i, cur_label in enumerate(label_ids_punc):
               ...
               for j, m in enumerate(cur_label):
                    if valid_ids[i][j]:  # if it's a valid label
                         temp_1.append(label_map_punc[m])
